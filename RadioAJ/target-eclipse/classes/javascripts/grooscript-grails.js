...
}).call(this);//This script needs grooscript.js to run
...
function RemoteDomain() {
  ...
  gSobject.closure = function(it) {
    ...
    return gs.execStatic(GrooscriptGrails,'remoteDomainAction', this,[remoteData, gSobject.onSuccess, gSobject.onFail, gSobject.name]);
  };
  ...
  gSobject['sendMessage'] = function(channel, data) {
    if (gSobject.mapHandlers [ channel]) {
      return gs.mc(gSobject.mapHandlers [ channel],"each",[function(action) {
        return (action.delegate!=undefined?gs.applyDelegate(action,action.delegate,[data]):gs.execCall(action, this, [data]));
      }]);
    };
  }
